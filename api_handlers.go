...
func GetDataHandler(c *gin.Context) {
	...
	// Unmarshal JSON into the metadata structure
	var metadata StoredMetadata
	if err := json.Unmarshal(jsonData, &metadata); err != nil {
		// It might be the old format (StoredData) if it's text data
		var oldData StoredData
		var errOld error                                                // Declare errOld outside the if scope
		if errOld = json.Unmarshal(jsonData, &oldData); errOld == nil { // Use = for assignment
			// If old format, construct metadata response (OriginalFilename will be empty)
			log.Printf("Retrieved legacy text data for ID %s, returning metadata only.", id)
			c.JSON(http.StatusOK, StoredMetadata{
				ID:               id, // Use the ID from the request param
				IV:               oldData.IV,
				Salt:             oldData.Salt,
				OriginalFilename: oldData.OriginalFilename, // Should be empty
			})
			return
		}
		// If neither format works, it's an error
		log.Printf("Error unmarshaling metadata for ID %s: %v (also failed legacy format: %v)", id, err, errOld) // Now errOld is accessible
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to parse stored metadata"})
		return
	}
	...
	log.Printf("Successfully retrieved metadata for ID: %s", id)
	// Return only the metadata (IV, Salt, OriginalFilename)
